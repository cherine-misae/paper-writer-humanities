...
Write each source note and section atomically. Discovery searches may run concurrently, but merge source records sequentially to avoid duplicate keys. Never let concurrent workers edit the same bibliography or ledger.
