...
- **Contextual**: reliable material used only for historical, institutional, or background context. Newspaper articles, columns, interviews, and press coverage of a scholar's work belong here even when written by a scholar; they are not secondary scholarship and may not stand in for the scholarly work they report on.
...
Assign one to every source: `full-text`, `partial`, `abstract-only`, `metadata-only`, `user-provided`. Metadata or abstract access is never represented as reading. A fetched URL verifies only what was visible on it. A work known only through press coverage, a review, or another author's citation is `metadata-only` for that work; the coverage itself is a separate contextual source.
