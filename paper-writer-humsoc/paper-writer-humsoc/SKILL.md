...
When the user says files exist (transcripts, scans, datasets, drafts), locate them in the workspace before planning. If they are not found, say so and ask where they are; do not proceed as if they were present.

Ask the profile's §3 questions **in the order the profile lists them** — the first item is always the one most likely to change the evidence (edition and translation for A). Confirm the citation style in one line even when the default applies.

**Test-scope limits.** If the user sets ceilings (e.g. "≤6 sources, 2 analytical sections, ≤3,000 characters"), record them in the manifest and stop at the ceiling instead of the saturation judgment; note in the completion report that coverage was capped.
...
- **Report at step boundaries only.** Give a two- or three-line summary when a workflow step completes or when the user's decision is needed (profile choice, corpus approval, inaccessible evidence, test-scope ceiling reached). Do not narrate individual searches, file writes, or renders in chat; they are recorded in `search_progress.md` and `section_progress.md`.
- **Batch file writes.** When several files are created together (source notes, section skeletons, appendices), write them in one shell command rather than one tool call per file. Each file is still written whole; batching does not violate atomicity.
- **Render at milestones, not after every section.** After each section: update the ledger, assemble `report.md`, update progress. Render DOCX/PDF and inspect pages (a) once after the scholarly-context section, (b) once after the last analytical section, (c) at step 6. Add an intermediate render only for manuscripts over ~20 pages.
- **Stop at the ceiling.** When test-scope limits are set, do not exceed them to "improve" the report.
...
Write one Markdown section at a time. Before each: fix its question, local claim, ledger rows, counterpressure, and link to the thesis. After each: update the ledger, assemble `report.md`, update `section_progress.md`; render and inspect at the milestones named under Working style.
...
- A source read only in part, via a review, news report, or abstract is qualified **in the body text where it is used**, not only in a limitations section. Title, abstract, and front matter claim no edition, corpus, or access the run did not actually have.
